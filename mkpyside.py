...
import stat
import sys
from PySide.QtCore import (QCoreApplication, QProcess)

__version__ = '1.0.3'

if sys.platform.startswith("win"):
    PATH = os.path.join(os.path.dirname(sys.executable),
                        "Lib/site-packages/PySide/bin")
else:
    app = QCoreApplication([])
    PATH = unicode(app.applicationDirPath())
    del app
PYUIC4 = os.path.join(PATH, "pyside-uic") # e.g. PYUIC4 = "/usr/bin/pyuic4"
...
if not os.access(PYUIC4, os.F_OK):
    msg.append("failed to find pyside-uic; tried %s" % PYUIC4)
if not os.access(PYRCC4, os.F_OK):
    msg.append("failed to find pyside-rcc; tried %s" % PYRCC4)
if not os.access(PYLUPDATE4, os.F_OK):
    msg.append("failed to find pyside-lupdate; tried %s" % PYLUPDATE4)
if msg:
    print "\n".join(msg)
